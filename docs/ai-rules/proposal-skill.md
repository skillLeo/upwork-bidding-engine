# SKILL.md v3 — Upwork Proposal Drafting (SkillLeo / Hassam Mehmood)
...
Optimize for INTERVIEWS AND HIRES, not for reply count. A tactic that gets more
replies but closes no more contracts (documented: question/Loom openers lift
replies with zero lift in contracts closed) is not a real win here — the proof
point and the mini-plan are what move hires, and they are never sacrificed for
a punchier-sounding opener.

Since May 2026, Upwork's Uma Recruiter reads this proposal and the screening
answers and ranks them against the job post before a human ever sees them, on
every client plan. Write so BOTH readers are satisfied by the same text:
mirror the client's own named requirements in their own words (this is what
Uma matches on) while still reading like a specific human who read the brief
(this is what the client responds to). Never keyword-stuff — a proposal that
reads like SEO fails both readers at once.
...
LINE 1 - THE ENVELOPE (only line visible in the client's list; make it A-pile):
  ...
  quoting or closely paraphrasing at least one exact detail or named requirement
  from THIS post (this is also what Uma's evaluator matches on). Must be
  impossible to have written for any other job. If you open with a question,
  the question must be tied to that same real technical detail, never a generic
  "can we chat" - a decorative question earns nothing. No greeting like
...
LINE 3 - THE MINI-PLAN / METHOD:
  ...
  of the finished outcome. "Done =" must name something the client can SEE or
  TEST (a working feature, a passing check, a delivered artifact) - never a
  promise of effort or care ("no manual cleanup after", "runs perfectly" are
  promises, not observable outcomes; reject and rewrite those). This reduces
  client risk and shows visible effort.
...
- Portfolio/GitHub links are allowed ONLY where Upwork permits (inside the
  ...
  words, never pasted as a link and never as an off-platform contact. Offer one
  only where a 60-second screen walkthrough would add real value to THIS job
  (e.g. showing a similar working feature) - skip it for education-category
  clients, and favor a Vimeo mention over Loom for design/video work.

## RATE SIGNALING
Never state a discount, never apologize for being new, never mention price or
an hourly rate (Hard Rule 2 already forbids naming one). On a small fixed-price
job, the mini-plan may frame a scoped first milestone sized to what the client
already stated, without ever naming a number. Do not manufacture urgency around
price. A very-low-price signal reads as risk, not as a bargain - never write
anything that implies "I'm cheap," let the one proof point carry the value.

## FOLLOW-UP MESSAGES (if asked to draft one, after the human has already submitted)
Only draft a follow-up if an open message thread already exists (an interview,
or a client who replied). Never draft a cold second contact to a client who
has not opened one. Keep it to one short, low-friction line - a small update
or a direct question, never a repeat of the original pitch. Never suggest
withdrawing and re-submitting the original proposal to "reset" its position;
that loses queue position and, if it was boosted or already got an interview
request, cannot be resubmitted at all.

## FINAL SELF-CHECK CHECKLIST (all must pass before output)
[ ] Screening questions answered first, specifically, if any exist - none left
    as a bare "Yes" or "N/A".
